        ...
        <div className="min-h-screen bg-slate-50 dark:bg-slate-900 mesh-gradient figma-grid flex flex-col items-center justify-center p-6 transition-colors duration-500">
            <div className="w-full max-w-3xl space-y-10">
                <div className="flex justify-between items-end px-2">
                    <h2 className="text-brand-teal-600 dark:text-brand-teal-400 font-black text-xl sm:text-2xl uppercase tracking-widest">
                ...
                <Card className="animate-reveal !p-6 md:!p-16 bg-white dark:bg-slate-800 shadow-xl border-slate-100 dark:border-slate-700">
                    <h3 className="text-2xl md:text-4xl font-black text-slate-900 dark:text-white text-center mb-16 leading-tight tracking-tighter">
                    ...
                    <div className="space-y-4">
                        {currentQuestion.options.map(opt => (
                            <button
                                ...
                                className={`tap-target w-full text-left p-6 sm:p-8 rounded-[2.5rem] border-4 flex items-center gap-8 transition-all duration-300 ${
                            ...
                            >
                                ...
                                <span className={`text-xl sm:text-2xl font-black tracking-tight ${answers[currentQuestion.id] === opt.value ? 'text-white' : 'text-slate-900 dark:text-slate-200'}`}>{opt.text}</span>
                            </button>
                        ))}
                    </div>
                </Card>
                ...
                {currentIndex === questions.length - 1 && (
                    <div className="flex justify-center pt-8">
                        <Button 
                            ...
                            className="w-full py-6 text-2xl sm:py-8 sm:text-3xl shadow-2xl"
                        >
